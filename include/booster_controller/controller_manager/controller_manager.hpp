...
public:
  ...
  bool submit_trajectory(std::shared_ptr<TrajectoryGoalHandle> goal_handle);
  bool cancel_trajectory(std::shared_ptr<TrajectoryGoalHandle> goal_handle);

  bool submit_joints(const std::vector<JointCommandTarget>& targets);
  bool submit_torques(const std::vector<Joint::JointIndex>& joints, bool enable);

  bool submit_mode_transition(const TransitionCommand& command, float& out_delay);

  bool trajectory_busy() const;
  bool cancel_pending_trajectory(std::shared_ptr<TrajectoryGoalHandle> goal_handle);
  void abort_pending_trajectory(std::string_view reason);
...
private:
  ...
  ActiveController active_controller_ = ActiveController::none;
